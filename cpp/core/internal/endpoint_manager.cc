...
void EndpointManager::RemoveEndpointState(const std::string& endpoint_id) {
  NEARBY_LOG(VERBOSE, "RemoveEndpointState for endpoint %s",
             endpoint_id.c_str());
  auto item = endpoints_.find(endpoint_id);
  if (item != endpoints_.end()) {
    NEARBY_LOGS(VERBOSE) << "EndpointState found for id: " << endpoint_id;
    ...
    NEARBY_LOGS(VERBOSE) << "Workers terminated for id: " << endpoint_id;
  } else {
    NEARBY_LOGS(VERBOSE) << "EndpointState not found for id: " << endpoint_id;
  }
}
...
void EndpointManager::RegisterEndpoint(ClientProxy* client,
  ...
  RunOnEndpointManagerThread("register-endpoint", [this, client,
    ...
    if (endpoints_.contains(endpoint_id)) {
      NEARBY_LOG(WARNING, "Registering duplicate endpoint %s",
                 endpoint_id.c_str());
    ...
    // for the pong.
    endpoint_state.StartEndpointKeepAliveManager([this, client, endpoint_id]() {
      EndpointChannelLoopRunnable("KeepAliveManager", client, endpoint_id,
                                  [this](EndpointChannel* channel) {
...
void EndpointManager::UnregisterEndpoint(ClientProxy* client,
                                         const std::string& endpoint_id) {
  NEARBY_LOG(VERBOSE, "UnregisterEndpoint for endpoint %s",
             endpoint_id.c_str());
  CountDownLatch latch(1);
...
void EndpointManager::DiscardEndpoint(ClientProxy* client,
                                      const std::string& endpoint_id) {
  NEARBY_LOG(VERBOSE, "DiscardEndpoint for endpoint %s", endpoint_id.c_str());
...
void EndpointManager::RemoveEndpoint(ClientProxy* client,
                                     const std::string& endpoint_id,
                                     bool notify) {
  NEARBY_LOG(INFO, "RemoveEndpoint for endpoint %s", endpoint_id.c_str());
